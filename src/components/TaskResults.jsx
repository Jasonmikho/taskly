...
function SortableItem({ id, index, el, isEditing, onChange, onDelete, onToggleComplete }) {
  ...
  return (
    ...
    >
      <div style={{ flex: 1, textDecoration: el.completed ? 'line-through' : 'none' }}>
  ...
  function parseSubtasks(subtasks) {
    ...
    for (let i = 0; i < subtasks.length; i++) {
      ...
      if (/^\d+\./.test(normalizedLine) && isDurationOnly) {
        ...
        const body = nextLine;
        currentTimedBlock = { id: makeId('next'), type: 'timedBlock', title, body, bullets: [], completed: false };
      ...
      if (/^\d+\./.test(normalizedLine) && inlineDurationMatch) {
        ...
        currentTimedBlock = { id: makeId('inline'), type: 'timedBlock', title, body, bullets: [], completed: false };
        parsed.push(currentTimedBlock);
    ...
    return { elements: parsed, minTotal: Math.round(minTotal), maxTotal: Math.round(maxTotal) };
  }
  ...
  return (
    <div className="results-section">
      {sortableElements.length > 0 && (
        <div style={{ marginBottom: '1rem', fontWeight: 'bold', color: '#1e293b' }}>
      ...
      {totalMinutes[0] > 0 && (
        <div className="task-card" style={{ backgroundColor: '#ecfdf5', color: '#065f46', fontWeight: 'bold' }}>
