...
export default function SavedTasks({
      ...
      const normalizedSubtasks = Array.isArray(viewingTask.subtasks)
        ? viewingTask.subtasks.map(line => {
            if (typeof line === 'string') {
              ...
              return { ...line, completed: line.completed ?? false };
            }
            return null;
          }).filter(Boolean)
        : [];
  ...
  async function handleSave(updateExisting = true) {
    ...
    const isTitleDuplicate = tasks.some(t =>
      t.task.trim().toLowerCase() === editedTask.task.trim().toLowerCase() &&
          ...
          {!isEditing ? (
            <>
              ...
              {confirmDeleteId === viewingTask.id ? (
                <div style={{ marginTop: '1rem' }}>
                  <div style={{ color: '#ef4444', marginBottom: '0.5rem' }}>Are you sure you want to delete this task?</div>
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button className="submit-btn secondary" onClick={() => setConfirmDeleteId(null)}>Cancel</button>
                    <button className="submit-btn" style={{ backgroundColor: '#ef4444' }} onClick={() => confirmDelete(viewingTask.id)}>
                      Confirm Delete
                    </button>
                  </div>
                </div>
            ...
            <>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginTop: '1rem' }}>
                <button className="submit-btn" onClick={() => handleSave(true)}>Save Changes</button>
                <button className="submit-btn secondary" onClick={() => handleSave(false)}>Save as New</button>
                <button className="submit-btn secondary" onClick={handleCancelEdit}>Cancel</button>
              </div>
            </>
          )}
        </div>
            ...
            <div key={t.id} className="task-card" style={{ position: 'relative' }}>
              ...
              {confirmDeleteId === t.id ? (
                <div style={{ marginTop: '0.5rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button onClick={() => setConfirmDeleteId(null)} className="submit-btn secondary">Cancel</button>
                    <button onClick={() => confirmDelete(t.id)} className="submit-btn" style={{ backgroundColor: '#ef4444' }}>
                      Confirm
                    </button>
                  </div>
                </div>
