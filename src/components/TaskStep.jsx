    ...
    <div className="task-card">
      {isEditing ? (
            ...
            <>
              ...
              <div style={{ marginTop: '1rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
                  <span style={{ fontWeight: 'bold', fontSize: '0.9rem' }}>Bullet Points:</span>
                  <button
                ...
                {localBullets.map((bullet, bulletIdx) => (
                  <div key={bulletIdx} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.25rem' }}>
                    <span>•</span>
                    <input
                      type="text"
                      value={bullet}
        ...
        <>
          {el.type === 'timedBlock' && (
            <>
              <div style={{ fontWeight: 'bold', marginBottom: '0.25rem' }}>{el.title}</div>
              <div style={{ marginLeft: '0.5rem', color: '#334155', marginBottom: '0.5rem' }}>{el.body}</div>
              {el.bullets?.length > 0 && (
                <ul style={{ marginTop: '0.25rem', paddingLeft: '1.5rem' }}>
                  {el.bullets.map((b, i) => <li key={i}>{b}</li>)}
                </ul>
              )}
            </>
          )}
