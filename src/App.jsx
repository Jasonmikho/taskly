  ...
  function handleAIResponse(result, task, history = []) {
    ...
    if (isQuestion) {
      ...
      if (!extracted || extracted === '()') {
        if (fallback) {
          const raw = fallback.replace('PREVIEW_BREAKDOWN:', '').replace(/BREAKDOWN:/i, '').trim();
...
if (isBreakdown) {
  ...
  const modifiedLines = extraContext
    ? [`* Extra context: ${extraContext}`, ...normalized.split('\n')]
    : normalized.split('\n');
    ...
    <>
      ...
      {authMode && (
        <div style={{
          ...
          {authMode === 'login' && <Login setUser={setUser} switchToRegister={() => setAuthMode('register')} switchToForgot={() => setAuthMode('forgot')} onClose={() => setAuthMode(null)} />}
        ...
        <div className="app-container">
          ...
          <div className="app-nav">
            <div className="nav-wrapper">
              <div className="nav-buttons">
                ...
                <button onClick={() => { setActiveTab('saved'); setViewingSavedTask(null); }} className={`nav-btn ${activeTab === 'saved' ? 'active' : ''}`}>
                  <Archive size={18} /> Saved Tasks
                </button>
                <button onClick={() => { setActiveTab('calendar'); setViewingSavedTask(null); }} className={`nav-btn ${activeTab === 'calendar' ? 'active' : ''}`}>
            ...
            <div className="top-right-menu" ref={menuRef}>
              <div className="menu-button-wrapper">
                <button onClick={() => setShowMenu(prev => !prev)} className="nav-btn">☰</button>
                {showMenu && (
          ...
          <div className="app-content">
            {activeTab === 'saved' && (
              <SavedTasks
                tasks={savedTasks}
                onSelect={(t) => { setViewingSavedTask(t); setScreenState('results'); }}
              ...
              <>
                ...
                {screenState === 'skip-message' && (
                  <div className="skip-message-container">
                    <div className="skip-message" style={{ marginBottom: '1rem', padding: '1rem', backgroundColor: '#f3f4f6', borderRadius: '0.5rem', border: '1px solid #d1d5db', color: '#374151' }}>
                      <p>{skipMessage}</p>
                    </div>
